...
foreach ( $pn as $producto){
	...
	$msg .= '<tr id="tr_'.$x.'"><td ><img src="'.str_replace('<SIZE>','400',$producto->info->imageUrl).'" width="60px"></td><td>'.$producto->info->productLabel.'</td><td><input type="number" id="precio_'.$x.'" name="precio_'.$x.'" value="'.($producto->storeSku->pricing->specialPrice * 0.85).'"></td><td>
		...
		<input type="hidden" id="id_product_'.$x.'" name="id_product_'.$x.'" value="'.$producto->productId.'">$'.$producto->storeSku->pricing->specialPrice.'</td><td><a href="javascript:void(0);" onclick="actualizar(\''.$x.'\');">Insertar</a> <a href="javascript:void(0);" onclick="descartarN(\''.$x.'\');">Descartar</a></td></tr>';
	$x++;
}
$msg .= '</tbody><table>';
$resultado['msg'] = $msg;
